...
async def list_all_anonymous_intakes(
    ...
    """List all anonymous intake submissions with filtering (admin only)."""
    try:
...
async def get_anonymous_intake_details(
    ...
    """Get detailed anonymous intake information with full history (admin only)."""
    try:
...
async def update_anonymous_intake(
    ...
    """Update anonymous intake status, notes, and assignment (admin only)."""
    try:
        update_data = {}
        ...
        await AdminOperations.create_audit_log(
            user_id=user_id,
            action="UPDATE_ANONYMOUS_INTAKE",
            resource_type="anonymous_intake",
...
async def add_anonymous_intake_note(
    intake_id: str,
    note_text: str,
    note_type: Optional[str] = "general",
    user_id: str = Depends(require_admin())
):
    """Add a note to an anonymous intake (admin only)."""
    try:
...
async def get_anonymous_intake_notes(
    intake_id: str, user_id: str = Depends(require_admin())
):
    """Get all notes for an anonymous intake (admin only)."""
    try:
        # Verify intake exists
        intake = await db.get_anonymous_intake(intake_id)
        if not intake:
